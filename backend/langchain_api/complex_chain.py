...
def setup_memory():
    ...
    return {"memory": memory, "housing_memory": housing_memory}


def get_previous_facts(memory):
...
def get_previous_housing_facts(housing_memory):
...
def generate_lifestyles(memory, housing_memory, situation: str):
    previous_facts = get_previous_facts(memory)
    previous_housing_facts = get_previous_housing_facts(housing_memory)
    ...
    while True:
        try:
            ...
            lifestyles, facts, fact_categories, emojis, housing_facts = (
                generate_lifestyles(user_input)
            )
